...
namespace Shell.MailSync
{
    public class MailSyncLibrary
    {
        ...
        private void MoveFolder (ImapFolderWrapper fromFolder, ImapFolderWrapper toFolder, MessageList messages, Dictionary<string,string> parameters, bool directDelete)
        {
            int blocksize = 10;
            int i = 0;
            int count = messages.Count;
                    ...
                    try {
                        ...
                        if (needToCopy.Count > 0) {
                            if (channel.Operation == ChannelOperation.COPY) {
                                ...
                                MoveFolder (fromFolder, toFolder, needToCopy, channel.Parameters, directDelete: false);
                            } else if (channel.Operation == ChannelOperation.DELETE) {
                                MoveFolder (fromFolder, toFolder, needToCopy, channel.Parameters, directDelete: channel.FromAccount.IsDirectDeleteEnabled);
                            }
